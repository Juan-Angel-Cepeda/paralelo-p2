...
async function list(req,res,next){
    ...
    try{
        ...
        try{
            ...
            const cachedOrders = await redis_con.get('orders');
            if(cachedOrders){
                console.log('Datos de redis');
                return res.json(JSON.parse(cachedOrders));
            }else{
                conexion = await oracle.connectToDatabase();
                const result = await conexion.execute('select * from orders');
                await redis_con.set('orders',JSON.stringify(result.rows));
